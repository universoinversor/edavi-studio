...
import { SB_URL, getUser, loginRequired, userHeaders } from '@/lib/server/session';
...
export async function POST(request) {
  ...
  const rowRes = await fetch(`${SB_URL}/rest/v1/edavi_generations?local_id=eq.${encodeURIComponent(localId)}&select=outputs,archived,status`, {
  ...
  for (const [i, output] of (row.outputs || []).entries()) {
    ...
    if (!media.ok) return NextResponse.json({ detail: `Higgsfield ya no tiene el archivo ${i + 1} (${media.status}).` }, { status: 410 });
    ...
    const up = await fetch(`${SB_URL}/storage/v1/object/edavi-media/${path}`, {
      method: 'POST', headers: userHeaders(user, { 'Content-Type': type, 'x-upsert': 'true' }), body: bytes,
    });
    if (!up.ok) return NextResponse.json({ detail: `No se pudo guardar en Supabase (${up.status}).` }, { status: 502 });
    archived[i] = { type: output.type, url: `${SB_URL}/storage/v1/object/public/edavi-media/${path}` };
  }
