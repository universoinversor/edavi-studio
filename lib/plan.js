...
// Aplica un prompt de la biblioteca al modelo actual: texto, parámetros que el
// modelo admita y, si hay storyboard, varias tomas (o un texto combinado).
export function applyPromptEntry(model, previous, entry) {
  const props = model.schema.properties;
  let prompt = entry.prompt;
  const extra = { ...(entry.params || {}) };
  if (entry.shots?.length) {
    if (props.multi_prompt && props.multi_shots) {
      extra.multi_shots = true;
      extra.multi_prompt = entry.shots.map((s) => ({ prompt: s.prompt, duration: s.duration }));
    } else {
      prompt = [entry.prompt, ...entry.shots.map((s, i) => `Shot ${i + 1}: ${s.prompt}.`)].join(' ');
    }
  }
  return carryValues(model, { ...previous, ...extra, prompt });
}
