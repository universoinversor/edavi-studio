...
// Rescate: resultados completados de los últimos 7 días que aún no están en tu nube
// (Higgsfield solo garantiza conservarlos una semana). Uno a uno para no saturar.
const RETENTION = 7 * 24 * 3600 * 1000;
async function rescueUnarchived() {
  const pending = jobs.filter((j) => j.status === 'completed' && j.outputs?.length && !(j.archived || []).length && Date.now() - j.createdAt < RETENTION);
  for (const job of pending) {
    try { await archiveJob(job.localId); } catch { /* se reintenta en el próximo inicio de sesión */ }
  }
}

if (typeof window !== 'undefined') {
  window.addEventListener('edavi:user', (/** @type {any} */ e) => { if (e.detail?.userId) { load(); syncFromCloud(); } });
}
  ...
  try {
    ...
    if (!TERMINAL.has(result.status)) {
      ...
      if (result.status === 'completed') {
        startChain(localId);
        // Higgsfield conserva los resultados al menos 7 días: con sesión se copian a tu nube
        // para que la biblioteca y la galería pública no pierdan nada.
        if (currentUserId()) archiveJob(localId).catch(() => {});
      }
...
function startChain(localId) {
  ...
  const next = enqueue({ model, payload, group: job.group, label: COPY.composer.flowLabel, parent: localId });
...
async function trySubmit(localId) {
  ...
  try {
    ...
    patch(localId, { status: 'error', error: friendlyError(err), correlationId: err.correlationId || null });
  }
}

/**
 * @param {{ model: any, payload: any, estimate?: any, then?: any, group?: string|null, label?: string|null, parent?: string }} job
 */
...
// ---------- Operaciones en lote (biblioteca) ----------

export function removeJobs(localIds) {
  const ids = new Set(localIds);
  for (const id of ids) clearTimeout(timers.get(id));
  jobs = jobs.filter((j) => !ids.has(j.localId));
  persist();
  storage.deleteJobs([...ids]).catch(() => {});
}

export function setFavorites(localIds, favorite) {
  for (const id of localIds) {
    const job = find(id);
    if (job && job.favorite !== favorite) patch(id, { favorite });
  }
}
...
// Visibilidad en la galería pública: solo cambia la copia local (la nube la actualiza
// storage.setPublished, porque el guardado normal no toca esa columna).
export function setPublishedLocal(localId, published) {
  jobs = jobs.map((j) => (j.localId === localId ? { ...j, published } : j));
  persist();
}
