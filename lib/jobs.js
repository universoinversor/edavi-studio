...
import { cancelRequest, estimateCost, getRequestStatus, submitGeneration } from './client';
...
export function toggleFavorite(localId) {
  const job = find(localId);
  if (job) patch(localId, { favorite: !job.favorite });
}
...
export function clearFinished() {
  ...
  jobs = jobs.filter((j) => j.favorite || (!TERMINAL.has(j.status) && j.status !== 'error'));
  persist();
}
