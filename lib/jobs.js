...
import { archiveOutputs, cancelRequest, estimateCost, getRequestStatus, submitGeneration } from './client';
...
import { deleteJobs, fetchJobs, flushNow, saveJob } from './cloud';
...
function patch(localId, changes) {
  ...
  if (job && !(Object.keys(changes).length === 1 && 'pollDelay' in changes)) saveJob(job);
}
...
async function syncFromCloud() {
  ...
  for (const j of jobs) if (!remoteIds.has(j.localId)) saveJob(j);
...
function startChain(localId) {
  ...
  estimateCost(model.endpoint, payload).then((e) => setEstimate(next, e)).catch(() => {});
}
...
export async function archiveJob(localId) {
  await flushNow();
  const archived = await archiveOutputs(localId);
