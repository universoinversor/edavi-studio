...
import { createCharacter, getCharacter } from './client';
import { friendlyError } from './errors';
import { deleteCharacter, fetchCharacters, saveCharacter } from './cloud';
...
async function syncFromCloud() {
  ...
  for (const c of list) if (!remote.some((r) => r.id === c.id)) saveCharacter(c).catch(() => {});
...
export async function importCharacter(id, name, modelVersion) {
  load();
  const r = await getCharacter(id.trim());
