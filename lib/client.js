...
export const estimateCost = (endpoint, payload) => apiFetch(`estimate/${endpoint}`, { method: 'POST', body: payload, silent: true });
...
export async function archiveOutputs(localId) {
  const res = await fetch('/api/archive', { method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ localId }) });
  if (!res.ok) throw await readError(res);
  return (await res.json()).archived;
}
